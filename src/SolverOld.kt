/*private fun expandBlack(board: Board<Int>, offset: Int, toGo: MutableList<Int>): Boolean {
    fail = false
    this.board = board
    this.toGo = toGo
    ...
    fun process(offset: Int = offset): Boolean {
        do {
            ...
            changes += techn1() + techn2() + techn3() + techn4() + techn5()
        } while (changes > 0)
    ...
    if (offset >= 0) {
        if (board[offset] != YELLOW)
            return false
        board[offset] = BLACK
    }
    if (!process())
        return false
    if (board.filter { it == BLACK }.count() == blacks) {
    ...
    for (i in (offset + 1)..(board.size - 1)) {
        val result = expandBlack(board.copy(), i, toGo.toMutableList())
        if (result)
            return true
    }
